...
// 发送MCP请求
function sendMCPRequest(port, method, params = {}, id = 1) {
...
async function testFullMCPFlow() {
  ...
  async function runMCPTests() {
    ...
    try {
      ...
      // 2. 获取工具列表
      console.log('\n🧪 步骤2: 获取工具列表');
      const toolsResponse = await sendMCPRequest(serverPort, 'tools/list', {}, 2);
      ...
      const alertResponse = await sendMCPRequest(serverPort, 'tools/call', {
        ...
        arguments: {
          type: 'MAIN',
          script: "alert('🎉 完整MCP流程测试成功！端口冲突已解决！');"
        }
      }, 3);
...
testFullMCPFlow().catch(console.error);