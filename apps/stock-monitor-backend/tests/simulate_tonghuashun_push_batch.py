...
class TonghuashunSimulator:
    ...
    async def generate_historical_data(self, session: AsyncSession):
        ...
        for code in self.stock_codes:
            # 先删除旧数据
            stmt = delete(StockDaily).where(StockDaily.code == code)
            ...
            for day in range(total_days):
                ...
                try:
                    ...
                    if day % 100 == 0:
                        print(f"   已生成 {code}: {day}/{total_days} 天数据")
...
async def main():
    """主函数"""
    # 加载问财股票池
    from tests.load_wencai_stocks import load_wencai_stocks
    wencai_dir = '/Users/mac/StudioProjects/open-citycloud/projects/event-crawler/data/wencai'
    stock_codes = load_wencai_stocks(wencai_dir)
    ...
    print(f"📊 开始模拟同花顺推送测试（批量版）")
    print(f"   问财股票总数: {len(stock_codes)}")
    print(f"   测试股票数量: {len(test_stock_codes)}")
    print(f"   测试股票: {test_stock_codes}")
    print(f"   历史数据: 400天（2024-01-01 至 2025-11-19）")
    print(f"   测试天数: 20天（2025-11-20 至 2025-12-10）\n")
    
    simulator = TonghuashunSimulator(test_stock_codes)
