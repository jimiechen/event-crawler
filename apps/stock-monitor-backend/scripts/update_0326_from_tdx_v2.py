...
def main():
    ...
    try:
        ...
        logger.info(f"✅ 获取到数据")
        logger.info(f"数据形状: {df.shape if hasattr(df, 'shape') else 'N/A'}")
        logger.info(f"数据索引: {df.index if hasattr(df, 'index') else 'N/A'}")
        logger.info(f"数据列: {df.columns.tolist() if hasattr(df, 'columns') else 'N/A'}")
        ...
        for stock in stocks_0326:
            ...
            try:
                ...
                # 打印调试信息
                logger.debug(f"尝试获取 {stock_code_full} 的数据")
                logger.debug(f"df['Close'] 类型: {type(df['Close'])}")
                
                # 通达信返回的数据格式是多层索引
                # df[field][stock_code][date] 或 df.loc[date, stock_code]
                
                # 方法1: 使用loc
                close_series = df['Close'].loc[stock_code_full]
                logger.debug(f"close_series: {close_series}")
