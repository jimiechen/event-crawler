...
HISTORY_HTML_DIR = os.path.join(DATA_DIR, "mobile_history")
USER_AGENTS = [
...
def enrich_history_data():
    ...
    # Collect all unique match IDs from history arrays
    ...
    with sync_playwright() as p:
        ...
        for key in history_keys:
            ...
            logger.info(f"Processing array: {key} ({len(data[key])} items)")
            ...
            for idx, item in enumerate(data[key]):
                ...
                logger.info(f"[{idx+1}/{len(data[key])}] Processing Match ID: {match_id}")
                
                # 1. Fetch & Parse Handicap
                handicap_url = f"https://m.okooo.com/match/handicap.php?MatchID={match_id}&from=%2Fjczq%2F"
                handicap_path = os.path.join(HISTORY_HTML_DIR, match_id, "handicap.html")
                handicap_html = fetch_html(page, handicap_url, handicap_path)
                
                if handicap_html:
                    handicap_data = parser.parse_handicap(handicap_html)
                    item["handicap"] = handicap_data
                    logger.info(f"  Added {len(handicap_data)} handicap records")
                
                # 2. Fetch & Parse Euro Odds
                odds_url = f"https://m.okooo.com/match/odds.php?MatchID={match_id}&from=%2Fjczq%2F"
                odds_path = os.path.join(HISTORY_HTML_DIR, match_id, "odds.html")
                odds_html = fetch_html(page, odds_url, odds_path)
                
                if odds_html:
                    odds_data = parser.parse_odds(odds_html)
                    item["euro_odds"] = odds_data
                    logger.info(f"  Added {len(odds_data)} odds records")
                
                # Save progress periodically
                if (idx + 1) % 5 == 0:
                    with open(PROCESSED_FILE, 'w', encoding='utf-8') as f:
                        json.dump(data, f, ensure_ascii=False, indent=2)
                    logger.info("  Saved progress to JSON")
