 ...
 * Controls DeepSeek web interface (https://chat.deepseek.com)
 */

import { BasePageController, controllerRegistry } from '../page-controller';

export class DeepSeekAdapter extends BasePageController {
  // CSS Selectors for DeepSeek interface
  private readonly SELECTORS = {
    textarea: 'textarea[placeholder*="输入"], textarea[placeholder*="Message"], #chat-input',
    sendButton: 'button[type="submit"], button.send-button, button:has(svg)',
    uploadButton: '[aria-label="上传图片"], button[title="上传图片"], .upload-btn',
    fileInput: 'input[type="file"]',
    messageContainer: '.chat-container, .message-list, .ds-chat, #chat-container',
    lastMessage: '.ds-markdown:last-child, .message-content:last-child, .chat-message:last-child',
    newChatButton: '[aria-label="新建对话"], button:has-text("New Chat"), .new-chat-btn',
    loadingIndicator: '.ds-loading, .typing-indicator, .loading-dots, .generating'
  };
  ...
  async sendMessage(text: string, image?: string): Promise<string> {
    ...
    if (!this.isReady()) {
      throw new Error('DeepSeek页面未准备好，请等待页面加载完成');
    }
  ...
  getLastReply(): string {
    ...
    const selectors = [
      ...
      '.ds-chat-message:last-child .text',
  ...
  async newChat(): Promise<void> {
    ...
    if (newChatBtn) {
      ...
      window.location.href = 'https://chat.deepseek.com/a/chat/s/new';
    }
  }
  ...
  async getChatHistory(): Promise<any[]> {
    const messages: any[] = [];
    const messageElements = document.querySelectorAll('.ds-chat-message, .chat-message, .message');

    messageElements.forEach((el, index) => {
      const content = el.querySelector('.ds-markdown, .content, .text')?.textContent?.trim();
...
// Register adapter
controllerRegistry.register('deepseek', DeepSeekAdapter);

// Auto-initialize if on DeepSeek page
