    ...
    <div class="wencai-card">
      ...
      <div class="timeline-section">
        <div class="timeline-container">
          <div 
            ...
            :class="['timeline-item', { active: isSameDay(date, selectedDate) }]"
            @click="selectDate(date)"
          >
            <div class="date-label">{{ formatDate(date, 'MM-DD') }}</div>
            <div :class="['status-indicator', getStatusForDate(date)]"></div>
          </div>
        </div>
      </div>
      ...
      <div class="search-section">
        <textarea 
          ...
          readonly
        ></textarea>
        <button class="search-btn" @click="openWencaiSearch">
          <span>🔍</span> 问财搜索
        </button>
      </div>
...
import { getLastNWorkingDays, formatDate, getPreviousWorkingDay } from '../utils/date-utils';

// 状态管理
const timelineDays = ref<Date[]>([]);
const selectedDate = ref<Date>(new Date());
const operationHistory = ref<Record<string, { status: 'green' | 'red' | 'yellow', timestamp: number }>>({});
const searchQuery = ref('');

const QUERY_TEMPLATE = "2025年10月24日成交量是2025年10月23日成交量的2.5倍以上，非北交 非创业版，非科创版，非ST，概念 行业，2025年10月23日和2025年10月24日涨幅低于13%  收盘价低于25";
...
const getStatusForDate = (date: Date) => {
  ...
  const record = operationHistory.value[dateStr];
  
  if (record) {
    return record.status;
  }
  
  // If no record, check if it's in the past (missed)
...
const updateQuery = (date: Date) => {
  const targetDateStr = formatDate(date, 'YYYY年MM月DD日');
  // Logic to find previous working day
  const prevDate = getPreviousWorkingDay(date);
  const prevDateStr = formatDate(prevDate, 'YYYY年MM月DD日');
  
  // Replace in template
  // The template has "2025年10月24日" (later) and "2025年10月23日" (earlier)
  // We assume the later date in the template corresponds to `date` and earlier to `prevDate`
  
  // Actually, let's just use regex replacement based on the hardcoded template logic requested
  // "2025年10月24日" -> targetDateStr
  // "2025年10月23日" -> prevDateStr
  
  let newQuery = QUERY_TEMPLATE
    .replace(/2025年10月24日/g, targetDateStr)
    .replace(/2025年10月23日/g, prevDateStr);
...
const openWencaiSearch = async () => {
  ...
  operationHistory.value[dateStr] = {
    status,
    timestamp: Date.now()
  };
  ...
  // Open WenCai
  const url = `http://www.iwencai.com/unifiedwap/result?w=${encodeURIComponent(searchQuery.value)}`;
  window.open(url, '_blank');
};

const saveOperationHistory = async () => {
  await chrome.storage.local.set({ wencaiOperationHistory: operationHistory.value });
};

const loadOperationHistory = async () => {
  const result = await chrome.storage.local.get(['wencaiOperationHistory']);
  if (result.wencaiOperationHistory) {
    operationHistory.value = result.wencaiOperationHistory;
  }
};
...
onMounted(async () => {
  ...
  timelineDays.value = getLastNWorkingDays(new Date(), 10);
