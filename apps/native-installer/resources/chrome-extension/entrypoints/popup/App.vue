<template>
  <div class="popup-container">
    ...
    <div class="sidebar-section">
      <button class="sidebar-button" @click="openSidebar" :disabled="isOpeningSidebar">
        <VectorIcon />
        <span>{{ isOpeningSidebar ? '打开中...' : '打开侧边栏' }}</span>
      </button>
    </div>

    <!-- THS Panel Button Section -->
    <div class="ths-panel-section">
      <button class="ths-panel-button" @click="openThsPanel" :disabled="isOpeningThsPanel">
        <TabIcon />
        <span>{{ isOpeningThsPanel ? '打开中...' : '打开同花顺' }}</span>
      </button>
    </div>
...
const nativeServerPort = ref<number>(56889);
...
// THS Panel management
const isOpeningThsPanel = ref(false);
...
const openThsPanel = async () => {
  if (isOpeningThsPanel.value) return;
  isOpeningThsPanel.value = true;
  try {
    // eslint-disable-next-line no-undef
    ...
    console.error('打开同花顺面板失败:', error);
  } finally {
    isOpeningThsPanel.value = false;
  }
};
...
.ths-panel-button:hover:not(:disabled) {
  background: #d97706;
  box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);
}

.ths-panel-button:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
