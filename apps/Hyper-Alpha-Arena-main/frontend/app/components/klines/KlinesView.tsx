...
export default function KlinesView({ onAccountUpdated }: KlinesViewProps) {
  ...
  const [marketType, setMarketType] = useState<'hyperliquid' | 'stocks'>('hyperliquid')
  ...
  const fetchWatchlist = async () => {
    try {
      ...
      if (marketType === 'stocks') {
        const response = await fetch('http://localhost:8001/api/stocks/symbols')
        if (response.ok) {
  ...
  const fetchMarketData = async () => {
    try {
      if (marketType === 'stocks') {
        ...
        const response = await fetch(`http://localhost:8001/api/stocks/market?symbols=${symbolsParam}`)
        ...
        const formattedData = data.map((item: any) => ({
          symbol: item.symbol,
          price: item.price || 0,
          oracle_price: item.price || 0,
          change24h: item.change24h || 0,
          volume24h: item.volume || 0,
          percentage24h: item.change24hPercent || 0,
          open_interest: 0,
          funding_rate: 0
        }))
      ...
      return (
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            {t('kline.backfillNotAvailable', 'Backfill is only available for Hyperliquid market')}
          </p>
        </div>
      )
    }
          ...
          <Card className="lg:col-span-2">
            <CardContent className="pt-4 space-y-3">
              <div className="flex items-center gap-2 mb-3">
                <button
                  onClick={() => setMarketType('hyperliquid')}
                ...
                >
                  {t('kline.hyperliquid', 'Hyperliquid')}
                </button>
                <button
