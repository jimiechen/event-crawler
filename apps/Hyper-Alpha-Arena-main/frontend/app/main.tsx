...
function App() {
  ...
  // Initialize default user and account immediately to avoid loading screen
  useEffect(() => {
    setUser({ id: 1, username: 'default' })
    setAccount({ id: 1, user_id: 1, name: 'Default Account', account_type: 'AI', initial_capital: 10000, current_cash: 10000, frozen_cash: 0 })
  }, [])

  // Disable WebSocket connection for stocks mode (no backend needed)
  const shouldConnectWebSocket = () => {
    return false // Disable WebSocket for stocks mode
  }
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      if (!shouldConnectWebSocket()) {
        console.log('WebSocket connection disabled for stocks mode')
        return
      }
      ...
      try {
        ...
        const timeoutId = setTimeout(() => {
          if (!hasConnected) {
            ...
            // Set minimal state for all modes
            setUser({ id: 1, username: 'default' })
            setAccount({ id: 1, user_id: 1, name: 'Default Account', account_type: 'AI', initial_capital: 10000, current_cash: 10000, frozen_cash: 0 })
          }
        }, 10000)
  ...
  if (!user || !account || (!effectiveOverview && tradingMode === 'paper')) return <div className="p-8">{connectionTimeout ? 'Server connection timeout. Some features may be limited.' : 'Connecting to trading server...'}</div>
